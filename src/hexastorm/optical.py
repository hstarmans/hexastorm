...
def find_largestcontour(img, denoise=False):
    ...
    denoise: if larger than 0 image gets denoised for erode and dilate operation
    ...
    if len(cntrs) == 0: raise Exception("Did not find a contour")
...
def spotsize(img, pixelsize = 3, debug=False, fname='detectsspotdebug.jpg'):
    ...
    returns dictionary with keys
        ...
        axes      short axis diameter, long axis diameter
                in micrometers
    '''
    #if img.max() == 255: print("Camera saturated, spotsize can be incorrect")
...
def cross_scan_error(img, pixelsize=3, debug=False, fname='crossscanndebug.jpg'):
    ...
    img: numpy array which should contain the line of which the width needs to be determined
    pixelsize: size of one pixel in micrometers
    debug: if true fit is plotted and stored in fname
    fname: name of file were result of debug is stored
    '''
    ...
    if debug: cv.imwrite(fname, skel*255)
