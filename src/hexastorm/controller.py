...
class Host:
    ...
    def __init__(self, test=False):
        ...
        if self.test:
            # maximum number spi communication attempts
            self.max_attempts = 1
            ...
            self.max_attempts = 3
            self.max_trials = 1e5
            # different syntax in micropython, np.float
    ...
    def init_micropython(self):
        ...
        self.i2c = I2C(scl=cfg.scl, sda=cfg.sda)
    ...
    def init_steppers(self, current=100):
        ...
        if not self.steppers_initialized:
            failed = False
            ...
            self.steppers_initialized = not failed
    ...
    def reset(self):
        ...
        sck = Pin(cfg.spi["sck"], Pin.IN)
        mosi = Pin(cfg.spi["mosi"], Pin.IN)
        ...
        sleep(1)
        self.init_micropython()
        command = [0] * (Spi.word_bytes + Spi.command_bytes)
    ...
    #     """
    #     if self.micropython:
    #         data = list(self.bus.readfrom_mem(self.platform.ic_address, 0, 1))[0]
    #     else:
