...
class LaserheadTest(Base):
    ...
    def spinprism(self, time_sec=15):
        "Spin prism for time_sec seconds."
        host = self.host
        host.enable_comp(polygon=True)
        logger.info(f"Spinning prism for {time_sec} seconds")
        sleep(time_sec)
        host.enable_comp(polygon=False)
        self.assertFalse((host.fpga_state)["error"])
    ...
    def lasertest(self, laser1=True):
        ...
        """
        host = self.host
        logger.info(f"Press enter to turn laser {1 if laser1 else 0} on")
        input()
        host.enable_comp(laser1=laser1, laser0=(not laser1))
        logger.info("Press enter to turn laser off")
        input()
        host.enable_comp(laser1=False, laser0=False)
        self.assertFalse((host.fpga_state)["error"])

    def test_diode(self, time_out=3):
        """Verify photodiode fires when laser & polygon run."""
        host = self.host
        self.assertFalse((host.fpga_state)["photodiode_trigger"])
        host.enable_comp(laser1=True, polygon=True)
        logger.info(f"Wait for diode trigger, {time_out} seconds")
        sleep(time_out)
    ...
    def test_stable(self, timeout=3):
        ...
        logger.info(f"Wait for synchronization, {timeout} seconds")
        sleep(timeout)
        self.assertTrue((host.fpga_state)["photodiode_trigger"])
        host.enable_comp(synchronize=False)
        self.assertFalse((host.fpga_state)["error"])

    def test_move(self, dist=10, steps_line=1, time_out=3):
        ...
        host.enable_comp(synchronize=True)
        start_pos = (host.position).copy()
        host.enable_steppers = True
        logger.info(f"Wait for synchronization, {time_out} seconds.")
        sleep(time_out)
        line = [0] * laz_tim["scanline_length"]
        for direction in [0, 1]:
            for _ in range(numb_lines):
                host.write_line(line, steps_line, direction)
            logger.info(f"Wait for move to complete, {time_out} seconds.")
            sleep(time_out)
            indx = list(mt_cfg["steps_mm"].keys())
            indx = indx.index(mt_cfg["orth2lsrline"])
            # assume y axis is 1
            start_pos[indx] += dist if direction else -dist
            assert_array_almost_equal((host.position), start_pos, decimal=1)
        host.write_line([])
        logger.info(f"Wait for stopline to execute, {time_out} seconds")
        sleep(time_out)
        self.assertFalse((host.fpga_state)["error"])
        host.enable_comp(synchronize=False)
    ...
    def test_scanline(self, numb_lines=1_000, repeat=True, singlefacet=False):
        ...
        host.enable_comp(singlefacet=singlefacet)
        # Pre-load buffer a bit
        for _ in range(8):
            host.write_line(line)
        logger.info(f"Processing {numb_lines} lines")
        sleep(3)
        start_ms = ticks_ms()
        if repeat:
            host.write_line(line, repetitions=numb_lines)
        else:
            for _ in range(numb_lines):
                host.write_line(line)
...
class MoveTest(Base):
    ...
    def read_pin(self):
        ...
        try:
            while True:
                st = self.host.fpga_state
                logger.info(f"[x, y, z] = [{st['x']}, {st['y']}, {st['z']}]")
                sleep(1)
