...
class TestSPI(SPIGatewareTestCase):
    ...
    async def test_readfrominterface(self, sim):
        ...
        for _ in range(4):
            ...
            read_data = await self.spi_exchange_data(write_data)
...
class TestMEM(LunaGatewareTestCase):
    ...
    async def test_writeandread(self, sim):
        ...
        self.assertEqual(sim.get(dut.space_available), self.FRAGMENT_ARGUMENTS["depth"])
        ...
        self.assertEqual(
            sim.get(dut.space_available), self.FRAGMENT_ARGUMENTS["depth"] - iterations
        )
